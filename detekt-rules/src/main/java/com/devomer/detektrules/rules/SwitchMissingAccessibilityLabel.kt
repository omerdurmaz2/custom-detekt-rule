...
class SwitchMissingAccessibilityLabel(config: Config = Config.empty) : Rule(config) {

    override val issue = Issue(
        javaClass.simpleName, // Kural ID'si: SwitchMissingContentDescriptionRule
        Severity.Warning,
        "For accessibility, a `Switch` composable must have a `contentDescription` provided via `Modifier.semantics`.",
        Debt.FIVE_MINS
    )

    private val switchComposableName = "Switch"
    ...
    override fun visitCallExpression(expression: KtCallExpression) {
        ...
        // 1. Ziyaret edilen çağrı bir "Switch" Composable mı?
        if (expression.calleeExpression?.text != switchComposableName) {
            return
        }

        // 2. Switch'in "modifier" parametresini bul.
        ...
        report(
            CodeSmell(
                ...
                "This `Switch` is missing a `contentDescription`. For accessibility, describe what this switch controls using `Modifier.semantics`."
            )
        )
    }
